...
import { PlusCircle, LayoutList, BarChart3, Info, ShieldCheck, } from 'lucide-react';
import { motion, useReducedMotion } from 'motion/react';
import { paperSpring, instantFade } from '../motion/tokens';
import { useAuth } from '../context/AuthContext';
import { SageLogo } from './SageLogo';
export const Navbar = React.memo(({ currentView, onNavigate, totalComplaintsCount, }) => {
    const { activeRole, openAuthModal } = useAuth();
    const prefersReduced = useReducedMotion();
    const isAdminRole = activeRole === 'admin' || activeRole === 'head_admin';
    return (<motion.header initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} transition={prefersReduced ? instantFade : paperSpring} className="sticky top-0 z-30">
      {/* Slim protocol strip */}
      ...
      <div className="bg-surface/85 backdrop-blur-xl border-b border-line">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3 flex flex-col lg:flex-row items-center justify-between gap-3">
          <div className="flex items-center gap-3 cursor-pointer group shrink-0" onClick={() => onNavigate('landing')} title="Return to S.A.G.E. Overview">
          ...
          <nav className="flex items-center gap-1 overflow-x-auto max-w-full scrollbar-none -mx-1 px-1 pb-1 sm:pb-0">
            {!isAdminRole && (<NavButton id="nav-landing-btn" active={currentView === 'landing'} onClick={() => onNavigate('landing')}>
                <Info className="w-3.5 h-3.5"/><span>How It Works</span>
              </NavButton>)}
            {!isAdminRole && (<NavButton id="nav-submit-btn" active={currentView === 'submit' || currentView === 'confirmation'} onClick={() => onNavigate('submit')}>
            ...
            <button id="nav-role-switcher-btn" type="button" onClick={openAuthModal} className="px-3 py-2 rounded-xl border border-line-strong bg-surface text-ink-soft hover:border-accent hover:text-accent-deep hover:-translate-y-px shadow-soft transition-all flex items-center gap-1.5 cursor-pointer font-mono text-[10px] font-bold uppercase tracking-[0.1em]" title="Switch Firebase Auth Role">
...
const NavButton = ({ id, active, onClick, children }) => (<button id={id} type="button" onClick={onClick} className={`relative px-3.5 py-2 rounded-xl font-mono text-[10px] font-bold uppercase tracking-[0.1em] transition-colors cursor-pointer flex items-center gap-1.5 whitespace-nowrap ${active ? 'text-accent-deep' : 'text-ink-soft hover:text-ink'}`}>
    {active && (<motion.span layoutId="nav-active-pill" className="absolute inset-0 rounded-xl bg-accent-soft border border-accent/30" transition={{ type: 'spring', stiffness: 320, damping: 30 }}/>)}
