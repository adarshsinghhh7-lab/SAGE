...
import { MapPin, Clock, ArrowRight, Link2 } from 'lucide-react';
...
export const ComplaintCard = React.memo(({ complaint, onUpvote, onSelect, }) => {
    ...
    const rotateX = useSpring(useTransform(tiltY, [-0.5, 0.5], [(prefersReduced || !canHover) ? 0 : 3, (prefersReduced || !canHover) ? 0 : -3]), { stiffness: 300, damping: 30 });
    const rotateY = useSpring(useTransform(tiltX, [-0.5, 0.5], [(prefersReduced || !canHover) ? 0 : -3, (prefersReduced || !canHover) ? 0 : 3]), { stiffness: 300, damping: 30 });
    ...
    return (<motion.article id={`complaint-card-${compId}`} onClick={handleCardClick} layout variants={{
        ...
        }} whileHover={prefersReduced ? {} : { borderColor: 'rgba(95, 122, 102, 0.35)', boxShadow: RAISED_SHADOW, transition: prefersReduced ? instantFade : paperSpring }} onMouseEnter={() => setIsHovered(true)} onMouseMove={handleMouseMove} onMouseLeave={handleMouseLeave} animate={{ boxShadow: isHovered ? RAISED_SHADOW : RESTING_SHADOW }} transition={prefersReduced ? instantFade : paperSpring} className="relative bg-surface border border-line rounded-2xl p-5 sm:p-6 cursor-pointer group paper-grain" style={{
            borderLeftWidth: '4px',
      ...
      <div className="flex items-start justify-between gap-3 mb-4">
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {isUrgent && (<span className="inline-flex items-center gap-1 px-2.5 py-0.5 text-[9px] font-mono font-bold uppercase tracking-[0.12em] rounded-full bg-clay-soft border border-clay/40 text-clay-deep">
      ...
      <div className="pt-4 border-t border-line flex items-center justify-between gap-3">
        <motion.button id={`upvote-btn-${compId}`} type="button" disabled={complaint.hasUpvoted} whileTap={prefersReduced ? {} : { scale: 0.97, transition: microTap }} onClick={handleUpvoteClick} className={`inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full text-[10px] font-mono font-bold uppercase tracking-wider transition-all cursor-pointer border ${complaint.hasUpvoted
        ...
        <div className="flex items-center gap-2 ml-auto">
          <button type="button" onClick={handleCopyLink} className={`inline-flex items-center gap-1 px-2 py-1 rounded-full text-[9px] font-mono font-bold uppercase tracking-wider border transition-all cursor-pointer ${linkCopied
            ? 'bg-accent text-white border-accent'
            : 'bg-surface text-ink-faint border-line-strong hover:border-bronze hover:text-bronze-deep'}`} title="Copy public tracking link">
