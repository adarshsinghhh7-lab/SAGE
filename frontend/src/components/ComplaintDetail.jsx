...
import { ArrowLeft, MapPin, Clock, CheckCircle2, Copy, Image as ImageIcon, Video as VideoIcon, ExternalLink, ShieldAlert, } from 'lucide-react';
import { motion, useReducedMotion } from 'motion/react';
import { paperSpring, instantFade } from '../motion/tokens';
import { getCategoryBadgeStyle, formatCategoryLabel, formatTimeAgo, getCategoryTabColor, } from '../utils/formatters';
...
export const ComplaintDetail = ({ complaint, onBackToFeed, onUpvote, onOpenImage, onGoToSubmit, }) => {
    ...
    return (<motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={prefersReduced ? instantFade : paperSpring} className="max-w-4xl mx-auto py-10 sm:py-14 px-4 sm:px-6">
      {/* Navigation Breadcrumb */}
      <div className="mb-8 flex flex-wrap items-center justify-between gap-4">
        <button id="back-to-feed-btn" type="button" onClick={onBackToFeed} className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-surface border border-line-strong text-ink-soft hover:text-ink hover:border-accent hover:-translate-y-px transition-all cursor-pointer shadow-soft">
      ...
      {/* Case file plate */}
      <div className="bg-surface border border-line rounded-2xl shadow-soft paper-grain overflow-hidden" style={{ borderTop: `5px solid ${catColor}` }}>
        {/* Header strip */}
        <div className="px-6 py-5 border-b border-line flex flex-wrap items-center justify-between gap-3">
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {isResolved && (<span className="font-mono text-[10px] text-accent-deep font-bold uppercase tracking-wider">Resolved</span>)}
          </div>

          <button type="button" onClick={copyIdToClipboard} className="inline-flex items-center gap-1.5 text-[10px] font-mono font-bold uppercase tracking-wider text-ink-faint hover:text-bronze-deep bg-transparent border border-line-strong rounded-lg px-2.5 py-1.5 transition-colors cursor-pointer">
        ...
        <div className="px-6 py-5 sm:px-8 sm:py-6">
          ...
          {complaint.photoUrl && (<div className="mt-7">
              <h2 className="s-mono-micro mb-2">Attached Photographic Evidence</h2>
              <div onClick={() => onOpenImage?.(complaint.photoUrl, `${compId} - ${formatCategoryLabel(complaint.category)}`)} role="button" tabIndex={0} onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ')
            onOpenImage?.(complaint.photoUrl, `${compId} - ${formatCategoryLabel(complaint.category)}`); }} className="group bg-moss border border-moss rounded-xl p-3 cursor-pointer overflow-hidden max-w-lg shadow-soft">
          ...
          {complaint.videoUrl && (<div className="mt-7">
              ...
              <div className="group bg-moss border border-moss rounded-xl p-3 overflow-hidden max-w-lg shadow-soft">
                <video src={complaint.videoUrl} controls preload="metadata" className="max-h-72 w-full object-contain mx-auto rounded-lg group-hover:opacity-90 transition-opacity"/>
                <div className="mt-2.5 text-center text-xs font-mono text-[#EDE7D8] flex items-center justify-center gap-1.5">
        ...
        {/* Resolution notes */}
        {isResolved && complaint.resolutionNotes && (<motion.div initial={{ opacity: 0, y: 14 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: '-40px' }} transition={prefersReduced ? instantFade : paperSpring} className="px-6 pb-7 sm:px-8">
        ...
        {isHeadAdmin && (<motion.div initial={{ opacity: 0, y: 14 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: '-40px' }} transition={prefersReduced ? instantFade : paperSpring} className="px-6 pb-7 sm:px-8">
            <div className="border border-clay/35 bg-clay-soft rounded-xl p-4">
              ...
              <button type="button" onClick={() => setIsRevealModalOpen(true)} className="s-btn s-btn-danger font-mono text-xs font-bold uppercase tracking-wider cursor-pointer">
        ...
        <div className="px-6 py-6 sm:px-8 border-t border-line flex flex-col sm:flex-row items-center justify-between gap-4 bg-surface-soft/60">
          <button id={`detail-upvote-btn-${compId}`} type="button" disabled={complaint.hasUpvoted} onClick={() => !complaint.hasUpvoted && onUpvote(compId)} className={`px-6 py-3 rounded-xl font-mono text-xs font-bold uppercase tracking-wider border transition-all cursor-pointer flex items-center justify-center gap-2.5 ${complaint.hasUpvoted ? 'bg-accent text-white border-accent opacity-90' : 'bg-ink text-white border-ink hover:bg-moss-deep hover:border-moss-deep'}`}>
            {complaint.hasUpvoted ? (<><CheckCircle2 className="w-4 h-4"/><span>{complaint.upvoteCount} Endorsed</span></>) : (<><span>Endorse Grievance ({complaint.upvoteCount || 0})</span></>)}
          </button>
          ...
          <div className="flex items-center gap-2.5">
            ...
            {!isAdminRole && (<button type="button" onClick={onGoToSubmit} className="s-btn s-btn-ghost font-mono text-xs font-bold uppercase tracking-wider cursor-pointer">Lodge Another</button>)}
          </div>
        </div>
      </div>

      <RevealIdentityModal complaintId={compId} complaintRef={compId} isOpen={isRevealModalOpen} onClose={() => setIsRevealModalOpen(false)}/>
    </motion.div>);
};
