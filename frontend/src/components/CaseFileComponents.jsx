...
export const TallyMarks = ({ count, className = '' }) => {
    if (count <= 0)
        return <span className={`font-mono text-[10px] text-ink-faint ${className}`}>0</span>;
    if (count > 20) {
...
export const FolderTabCard = ({ category, children, className = '', ...props }) => {
    const tabColor = getCategoryTabColor(category);
    return (<div className={`relative overflow-hidden ${className}`} style={{ borderLeft: `4px solid ${tabColor}` }} {...props}>
      {children}
    </div>);
};
...
export const ComplaintIdStamp = ({ id, className = '' }) => {
    const hash = simpleHash(id || '');
    const rotation = ((hash % 5) - 2); // -2 to +2 degrees
    return (<span className={`font-mono text-[10px] font-bold text-ink-faint tracking-wider ${className}`} style={{ transform: `rotate(${rotation}deg)`, display: 'inline-block' }}>
      {id}
    </span>);
};
/* StatusStamp - soft pill treatment for status labels. */
export const StatusStamp = ({ status, color, className = '' }) => {
    const colorMap = {
        submitted: '#7D868F',
        under_review: '#AD8B5B',
        resolved: '#5F7A66',
    ...
    return (<span className={`inline-flex items-center gap-1 px-2.5 py-1 font-mono text-[10px] font-bold uppercase tracking-[0.12em] border rounded-full ${className}`} style={{
            color: stampColor,
...
export const IdentitySealedBar = ({ className = '', isSandbox = false }) => (<div className={`identity-sealed-bar ${className}`}>
    <Lock className="w-3 h-3 text-current"/>
    <span>Identity: server-sealed</span>
    {isSandbox && (<span className="ml-1.5 text-[9px] font-mono uppercase font-bold tracking-wider bg-clay-soft text-clay-deep border border-clay/40 px-1.5 py-0.5 rounded-full">
        Demo Sandbox Record
      </span>)}
  </div>);
/* PriorityStamp - clay PRIORITY pill over card corner with a light spring settle. */
export const PriorityStamp = ({ className = '' }) => {
    const prefersReduced = useReducedMotion();
    return (<motion.span initial={{ scale: 1.5, opacity: 0, rotate: -10 }} animate={{ scale: 1, opacity: 1, rotate: -3 }} transition={prefersReduced ? instantFade : { type: 'spring', stiffness: 340, damping: 20, mass: 0.6 }} className={`absolute -top-2 -right-2 z-10 inline-flex items-center gap-1.5 px-2.5 py-1 font-mono text-[10px] font-bold uppercase tracking-[0.12em] border border-clay/50 text-clay-deep bg-clay-soft rounded-full shadow-lift ${className}`}>
