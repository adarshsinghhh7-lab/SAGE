...
export const AuthModal = () => {
    ...
    if (!isAuthModalOpen)
        return null;
    const handleSelectRole = async (role) => { await loginAsDemoRole(role); };
    return (<AnimatePresence>
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} transition={prefersReduced ? instantFade : { duration: 0.15 }} className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 bg-black/70 backdrop-blur-xs overflow-y-auto" onClick={closeAuthModal}>
        <motion.div initial={{ opacity: 0, scale: 0.94, y: 10 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.94, y: 10 }} transition={prefersReduced ? instantFade : heavyDrawer} className="bg-surface border border-line-strong max-w-lg w-full my-8 overflow-hidden paper-grain rounded-2xl shadow-lift" onClick={(e) => e.stopPropagation()} role="dialog" aria-modal="true">
