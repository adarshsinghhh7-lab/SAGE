...
function resolveStageTimestamp(stage, submittedAt, resolvedAt, statusUpdates) {
    if (stage === 'submitted')
...
export const StatusTimeline = ({ currentStatus, submittedAt, statusUpdates = [], resolvedAt }) => {
    ...
    return (<div className="bg-surface border border-line rounded-2xl p-6 sm:p-7 shadow-soft paper-grain">
      ...
      <div className="relative">
        ...
        <motion.div className="absolute top-5 left-6 hidden sm:block h-0.5 bg-bronze rounded-full" initial={{ width: '0%' }} animate={{ width: `${progressFraction * 100}%` }} transition={prefersReduced ? instantFade : paperSpring} style={{ maxWidth: 'calc(100% - 3rem)' }}/>
          ...
          {STAGES.map((stage, idx) => {
            ...
            const labelClass = isCurrent
                ...
                : isCompleted
                    ? 'text-accent-deep'
                    : 'text-ink-faint';
            return (<motion.li key={stage.key} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={prefersReduced ? instantFade : { ...paperSpring, delay: idx * 0.08 }} className="relative z-10 flex-1 flex flex-col sm:items-center sm:text-center px-1">
                <div className={`relative z-10 w-10 h-10 rounded-full border-2 flex items-center justify-center mb-3 shadow-soft ${dotClass}`}>
                  {isCompleted && (<motion.span initial={{ scale: 0.6, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={prefersReduced ? instantFade : { ...paperSpring, delay: 0.3 + idx * 0.08 }} className="flex items-center justify-center">
                      {isCompleted && !isCurrent ? <Check className="w-4 h-4"/> : stage.icon}
                    </motion.span>) || stage.icon}
                </div>
