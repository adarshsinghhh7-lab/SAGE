...
export const RevealIdentityModal = ({ complaintId, complaintRef, isOpen, onClose, onRevealed, }) => {
    ...
    if (!isOpen || activeRole !== 'head_admin')
        return null;
    const handleConfirm = async () => {
        ...
        try {
            const result = await ApiService.triggerIdentityReveal(complaintId, trimmed, activeRole, user?.uid, user?.displayName ||
                user?.email ||
                (activeRole === 'head_admin' ? 'Chief Proctor (Head Admin)' : 'Head Admin'), token);
            ...
            if (/not found/i.test(raw)) {
                setRevealError(`Record '${complaintId}' is not present in the backend ledger — it likely only exists in the local browser cache. Refresh the page to re-sync with the live ledger, then try again.`);
            }
    ...
    return (<AnimatePresence>
      {isOpen && (<motion.div key="reveal-backdrop" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} transition={prefersReduced ? instantFade : { duration: 0.18, ease: 'easeOut' }} className="fixed inset-0 z-50 flex items-start sm:items-center justify-center p-0 sm:p-4 modal-depth-backdrop" role="dialog" aria-modal="true" aria-labelledby="reveal-modal-title" onClick={onClose}>
          <motion.div key="reveal-card" initial={{ opacity: 0, scale: 0.94 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.94 }} transition={prefersReduced ? instantFade : { ...heavyDrawer, delay: 0.05 }} className="w-full max-w-md bg-surface border border-clay/40 overflow-hidden paper-grain shadow-lift rounded-2xl max-sm:min-h-dvh max-sm:h-dvh max-sm:overflow-y-auto" style={{ boxShadow: '0 12px 28px rgba(11,12,15,0.16), 0 4px 10px rgba(11,12,15,0.08)' }} onClick={(e) => e.stopPropagation()}>
            {/* Header */}
            <div className="bg-ink text-surface px-5 py-4 flex items-center justify-between border-b border-clay/40 rounded-t-2xl">
              ...
              <button type="button" onClick={onClose} disabled={isRevealing} className="text-surface/70 hover:text-surface cursor-pointer disabled:opacity-40" aria-label="Close">
                <X className="w-5 h-5"/>
              </button>
            </div>
              ...
              {revealResult ? (<>
                  ...
                  <div className="relative bg-ink border border-line-strong rounded-xl p-4 mb-3 overflow-hidden">
                    ...
                    <div className="relative">
                      ...
                      <motion.div className="absolute inset-0" initial={{ clipPath: 'inset(0% 0% 0% 0%)' }} animate={{ clipPath: didUnmask ? 'inset(0% 0% 0% 100%)' : 'inset(0% 0% 0% 0%)' }} transition={prefersReduced ? instantFade : heavyDrawer} onAnimationComplete={() => setDidUnmask(true)}>
                  ...
                  <div className="mt-4 flex items-center justify-end">
                    <button type="button" onClick={onClose} className="px-4 py-2 bg-ink text-surface rounded-lg font-mono text-xs font-bold uppercase hover:opacity-90 cursor-pointer transition-opacity">
                      Close
                    </button>
                  </div>
                </>) : isRevealing ? (<>
                  ...
                  <motion.div initial={{ boxShadow: '0 0 0 0px rgba(188,108,86,0)' }} animate={{ boxShadow: ['0 0 0 0px rgba(188,108,86,0)', '0 0 0 5px rgba(188,108,86,0.18)', '0 0 0 0px rgba(188,108,86,0)'] }} transition={{ duration: 2.2, times: [0, 0.5, 1], ease: 'easeInOut', delay: 0.45 }}>
                    <textarea id="reveal-reason" value={reason} onChange={(e) => {
                    setReason(e.target.value);
                    if (reasonError)
                        setReasonError('');
                }} disabled={isRevealing} rows={3} placeholder="Enter the official investigation / legal justification for decryption..." className="w-full bg-surface border border-line-strong rounded-lg p-2.5 text-xs font-sans text-ink placeholder:text-ink-faint focus:outline-none focus:border-clay focus:ring-2 focus:ring-clay/20 disabled:bg-line/40"/>
                  </motion.div>
                  <p aria-live="polite" className={'mt-1 text-[10px] font-mono flex items-center gap-1 ' +
                  ...
                  {reasonError && (<p className="mt-1 text-xs font-mono text-clay-deep font-bold flex items-center gap-1">
                      <ShieldAlert className="w-3.5 h-3.5"/> {reasonError}
                    </p>)}
                  {revealError && (<p className="mt-1 text-xs font-mono text-clay-deep font-bold">{revealError}</p>)}

                  <div className="mt-4 flex items-center justify-end gap-2.5">
                    <button type="button" onClick={onClose} disabled={isRevealing} className="px-4 py-2 text-xs font-mono font-bold uppercase rounded-lg border border-line-strong bg-surface text-ink-soft hover:border-ink-soft transition-colors cursor-pointer disabled:opacity-40">
                      Cancel
                    </button>
                    <button type="button" onClick={handleConfirm} disabled={isRevealing || reason.trim().length < 10} className="px-4 py-2 bg-clay text-white rounded-lg font-mono text-xs font-bold uppercase hover:bg-clay-deep cursor-pointer disabled:bg-line disabled:text-ink-faint disabled:cursor-not-allowed flex items-center gap-1.5 transition-colors">
