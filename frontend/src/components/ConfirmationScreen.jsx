...
import { Copy, Check, ArrowRight, PlusCircle, MapPin, Clock, FileCheck, Link2 } from 'lucide-react';
...
export const ConfirmationScreen = ({ complaint, onGoToFeed, onSubmitAnother, }) => {
    ...
    return (<motion.div initial={{ opacity: 0, scale: 0.97 }} animate={{ opacity: 1, scale: 1 }} transition={prefersReduced ? instantFade : paperSpring} className="max-w-2xl mx-auto py-12 sm:py-16 px-4 sm:px-6">
      <motion.div initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} transition={prefersReduced ? instantFade : { ...paperSpring, delay: 0.08 }} className="bg-surface border border-line-strong p-8 sm:p-12 text-center paper-grain rounded-2xl shadow-lift">
        {/* Success Marker */}
        ...
        <div className="flex flex-col sm:flex-row items-center justify-center gap-3.5">
          <button id="copy-public-link-btn" type="button" onClick={copyPublicLink} className={`w-full sm:w-auto px-6 py-3.5 font-mono font-bold text-xs uppercase tracking-wider rounded-xl border transition-all flex items-center justify-center gap-2 cursor-pointer ${linkCopied ? 'bg-accent-deep text-white border-accent-deep' : 'bg-ink text-surface hover:bg-moss-deep border-ink'}`}>
          ...
          <button id="view-in-feed-btn" type="button" onClick={onGoToFeed} className="w-full sm:w-auto px-6 py-3.5 bg-ink hover:bg-moss-deep text-surface font-mono font-bold text-xs uppercase tracking-wider rounded-xl border border-ink transition-all flex items-center justify-center gap-2 cursor-pointer">
            <span>View in Public Ledger</span><ArrowRight className="w-4 h-4"/>
          </button>

          <button id="submit-another-btn" type="button" onClick={onSubmitAnother} className="w-full sm:w-auto px-6 py-3.5 bg-transparent hover:border-ink-soft hover:text-ink text-ink-soft font-mono font-bold text-xs uppercase tracking-wider rounded-xl border border-line-strong transition-colors flex items-center justify-center gap-2 cursor-pointer">
