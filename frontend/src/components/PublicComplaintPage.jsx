...
import { ArrowLeft, MapPin, Clock, Image as ImageIcon, Video as VideoIcon, ExternalLink, Link2, Loader2, AlertTriangle, Lock, } from 'lucide-react';
...
export const PublicComplaintPage = ({ complaintId, onExit, onOpenImage, }) => {
        ...
        async function load() {
            ...
            try {
                const [comp, updates] = await Promise.all([
                    ...
                    ApiService.getStatusUpdates(complaintId).catch(() => []),
                ]);
                if (cancelled)
                    return;
                if (!comp) {
    ...
    if (notFound || !complaint) {
        return (<div className="max-w-4xl mx-auto py-16 px-4">
    ...
    return (<div className="max-w-4xl mx-auto py-8 sm:py-12 px-4 sm:px-6">
      {toast && (<div className="fixed top-20 left-1/2 -translate-x-1/2 z-50 px-4 py-2.5 bg-ink text-surface text-xs font-mono font-bold uppercase tracking-wider border border-line-strong rounded-xl shadow-lift">{toast}</div>)}

      {/* Navigation */}
      <div className="mb-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
        <button type="button" onClick={onExit} className="inline-flex items-center gap-2 px-4 py-2.5 text-xs font-mono font-bold uppercase tracking-wider text-ink border border-line-strong bg-surface rounded-xl hover:border-bronze/50 hover:text-bronze-deep transition-colors shadow-soft cursor-pointer">
      ...
      <div className="bg-surface border border-line-strong rounded-2xl p-6 sm:p-10 shadow-lift paper-grain">
        ...
        <div className="mb-6 border-b border-line pb-6">
          <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
            <div className="flex items-center gap-2.5 flex-wrap">
              ...
              {isHighPriority && (<span className="inline-flex items-center gap-1 text-xs font-mono font-bold uppercase px-2.5 py-1 bg-clay-soft text-clay-deep border border-clay/50 rounded-full">HIGH PRIORITY</span>)}
        ...
        {/* Resolution Notes */}
        {isResolved && complaint.resolutionNotes && (<div className="mb-8">
        ...
        {/* Evidence Photo */}
        {complaint.photoUrl && (<div className="mb-8">
        ...
        {/* Evidence Video */}
        {complaint.videoUrl && (<div className="mb-8">
        ...
        <div className="pt-6 border-t border-line flex flex-col sm:flex-row items-center justify-between gap-4">
          <button id={`public-upvote-btn-${compId}`} type="button" disabled={complaint.hasUpvoted} onClick={handleUpvote} className={`px-6 py-3 text-xs font-mono font-bold uppercase tracking-wider rounded-xl border transition-all shadow-soft cursor-pointer flex items-center justify-center gap-2.5 ${complaint.hasUpvoted ? 'bg-accent text-surface border-accent' : 'bg-ink text-surface hover:bg-bronze hover:border-bronze border-ink'}`}>
            {complaint.hasUpvoted ? <span>{complaint.upvoteCount} · Endorsed</span> : <span>Endorse Grievance ({complaint.upvoteCount || 0})</span>}
          </button>
          <button type="button" onClick={copyShareLink} className={`px-5 py-3 text-xs font-mono font-bold uppercase tracking-wider rounded-xl border transition-colors text-center cursor-pointer ${copyState === 'copied' ? 'bg-accent text-surface border-accent' : 'bg-transparent text-ink-soft border-line-strong hover:border-bronze hover:text-bronze-deep'}`}>
