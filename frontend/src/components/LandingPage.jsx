...
import { ShieldCheck, Lock, EyeOff, PlusCircle, LayoutList, CheckCircle2, ChevronDown, Shield, Activity, AlertTriangle } from 'lucide-react';
...
export const LandingPage = ({ onNavigate, }) => {
    ...
    return (<div className="text-ink">
      ...
      <section className="py-16 sm:py-24 px-4 sm:px-6 lg:px-8 relative overflow-hidden bg-transparent">
        ...
        <motion.div className="max-w-5xl mx-auto text-center relative z-10" variants={{ visible: { transition: { staggerChildren: 0.15 } } }} initial="hidden" animate="visible" style={{ y: heroY }}>
          {/* Brand Emblem */}
          <motion.div variants={{ hidden: { opacity: 0, y: 20 }, visible: { opacity: 1, y: 0 } }} transition={prefersReduced ? instantFade : paperSpring} className="flex justify-center mb-8">
            <div className="relative w-[148px] h-[148px] sm:w-[164px] sm:h-[164px] flex items-center justify-center">
              ...
              {!prefersReduced && (<motion.div className="absolute inset-0" animate={{ rotate: 360 }} transition={{ duration: 90, ease: 'linear', repeat: Infinity }} aria-hidden="true">
              ...
              {!prefersReduced && (<motion.div className="absolute inset-3" animate={{ rotate: -360 }} transition={{ duration: 120, ease: 'linear', repeat: Infinity }} aria-hidden="true">
                  <svg viewBox="0 0 160 160" className="w-full h-full">
          ...
          {/* Trust Pill */}
          <motion.div variants={{ hidden: { opacity: 0, y: 20 }, visible: { opacity: 1, y: 0 } }} transition={prefersReduced ? instantFade : paperSpring} className="inline-flex items-center gap-2 px-4 py-1.5 text-xs font-mono font-bold uppercase tracking-wider mb-6 shadow-soft rounded-full bg-surface border border-line backdrop-blur-sm">
          ...
          {/* Main Title */}
          <motion.h1 variants={{ hidden: { opacity: 0, y: 20 }, visible: { opacity: 1, y: 0 } }} transition={prefersReduced ? instantFade : paperSpring} style={{ fontSize: "clamp(2rem, 5vw + 1rem, 4.5rem)" }} className="font-display font-semibold text-ink tracking-tight leading-[1.08] mb-6">
          ...
          {/* Subtitle */}
          <motion.p variants={{ hidden: { opacity: 0, y: 20 }, visible: { opacity: 1, y: 0 } }} transition={prefersReduced ? instantFade : paperSpring} className="text-lg sm:text-xl text-ink-soft max-w-3xl mx-auto leading-relaxed mb-12">
          ...
          <motion.div variants={{ visible: { transition: { staggerChildren: 0.1 } } }} className="grid grid-cols-1 sm:grid-cols-3 gap-3 pt-6 border-t border-line text-xs font-mono text-ink-soft">
            {[
        ...
        ].map((item, i) => (<motion.div key={i} variants={{ hidden: { opacity: 0, y: 10 }, visible: { opacity: 1, y: 0 } }} transition={prefersReduced ? instantFade : { ...paperSpring, delay: prefersReduced ? 0 : 0.45 + i * 0.08 }} whileHover={prefersReduced ? {} : { y: -2 }} className="flex items-center justify-center gap-2 p-2.5 bg-surface border border-line rounded-xl shadow-soft">
                {item.icon}
                <span>{item.text}</span>
              </motion.div>))}
          </motion.div>
      ...
      <section className="py-16 sm:py-24 px-4 sm:px-6 lg:px-8 border-b border-line" style={{ background: 'linear-gradient(180deg, #EFEBE1 0%, #EAE4D3 50%, #E2DBC6 100%)' }}>
        <div className="max-w-6xl mx-auto">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={prefersReduced ? instantFade : paperSpring} className="text-center max-w-3xl mx-auto mb-16">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: '-50px' }} transition={prefersReduced ? instantFade : { ...paperSpring, delay: 0 }} whileHover={prefersReduced ? {} : { y: -4 }} className="flat-paper p-6 flex flex-col justify-between">
              <div>
            ...
            <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: '-50px' }} transition={prefersReduced ? instantFade : { ...paperSpring, delay: 0.15 }} whileHover={prefersReduced ? {} : { y: -4 }} className="flat-paper p-6 flex flex-col justify-between">
              <div>
            ...
            <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: '-50px' }} transition={prefersReduced ? instantFade : { ...paperSpring, delay: 0.3 }} whileHover={prefersReduced ? {} : { y: -4 }} className="flat-paper p-6 flex flex-col justify-between">
              <div>
          ...
          <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: '-50px' }} transition={prefersReduced ? instantFade : { ...paperSpring, delay: 0.1 }} className="mt-14">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {/* Warning panel — a fake case can be traced back to its owner */}
              <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: '-50px' }} transition={prefersReduced ? instantFade : { ...paperSpring, delay: 0 }} whileHover={prefersReduced ? {} : { y: -4 }} className="flat-paper p-6 flex flex-col justify-between border-t-[3px] border-t-clay">
                <div>
            ...
            },
            {
        ...
        ].map((row) => (<div key={row.step} className="flex items-start gap-3 rounded-xl border border-clay/30 bg-clay-soft/50 px-3 py-2.5">
              ...
              <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: '-50px' }} transition={prefersReduced ? instantFade : { ...paperSpring, delay: 0.15 }} whileHover={prefersReduced ? {} : { y: -4 }} className="flat-paper p-6 flex flex-col justify-between border-t-[3px] border-t-accent">
                <div>
                  <div className="flex items-start justify-between gap-3 mb-4">
        ...
        ].map((point) => (<div key={point} className="flex items-start gap-2.5 text-[11px] sm:text-xs text-ink-soft leading-relaxed">
      ...
      <section className="py-16 sm:py-24 px-4 sm:px-6 lg:px-8 border-b border-line bg-surface-soft">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="space-y-4">
            ...
            return (<motion.div key={idx} initial={{ opacity: 0, y: 15 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: '-30px' }} transition={prefersReduced ? instantFade : { ...paperSpring, delay: idx * 0.08 }} className="flat-paper overflow-hidden">
                  <button type="button" onClick={() => toggleFaq(idx)} className="w-full p-5 text-left flex items-center justify-between gap-4 font-mono font-bold text-xs sm:text-sm text-ink hover:bg-bronze-soft transition-colors cursor-pointer" aria-expanded={isOpen}>
                    <span className="flex items-center gap-3">
                      <span className="text-bronze-deep font-mono text-sm">Q{idx + 1}.</span>
                      <span>{faq.question}</span>
                    </span>
                    <motion.span animate={{ rotate: isOpen ? 180 : 0 }} transition={prefersReduced ? instantFade : paperSpring} className="shrink-0">
                      <ChevronDown className="w-4 h-4 text-bronze-deep"/>
                    </motion.span>
                  </button>

                  <AnimatePresence initial={false}>
                    {isOpen && (<motion.div key="content" initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={prefersReduced ? instantFade : paperSpring} className="overflow-hidden">
        ...
        <div className="max-w-4xl mx-auto">
          <div className="ink-panel p-10 sm:p-14 text-center space-y-6 shadow-moss">
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center gap-4 pt-4">
              {!isAdminRole && (<motion.button type="button" onClick={() => onNavigate('submit')} whileTap={prefersReduced ? {} : { scale: 0.97, transition: microTap }} whileHover={prefersReduced ? {} : { y: -2, transition: paperSpring }} className="w-full sm:w-auto px-8 py-4 bg-bronze text-ink hover:bg-bronze-deep hover:text-[#EDE7D8] text-xs font-mono font-bold uppercase tracking-wider rounded-xl transition-all flex items-center justify-center gap-2 shadow-lg cursor-pointer">
                  <PlusCircle className="w-4 h-4"/>
                  <span>Submit a Complaint</span>
                </motion.button>)}

              <motion.button type="button" onClick={() => onNavigate('feed')} whileTap={prefersReduced ? {} : { scale: 0.97, transition: microTap }} whileHover={prefersReduced ? {} : { y: -2, transition: paperSpring }} className="w-full sm:w-auto px-8 py-4 bg-transparent text-[#EDE7D8] hover:text-white hover:bg-white/5 text-xs font-mono font-bold uppercase tracking-wider border border-white/20 hover:border-white/50 rounded-xl transition-all flex items-center justify-center gap-2 cursor-pointer">
