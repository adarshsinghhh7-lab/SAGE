...
import { Search, PlusCircle, RotateCcw, Inbox, Lock } from 'lucide-react';
...
export const PublicFeed = ({ complaints, onUpvote, onSelectComplaint, onOpenImage, onGoToSubmit, }) => {
    ...
    const filteredComplaints = useMemo(() => {
        return complaints
            ...
            if (selectedLocation !== 'All') {
                ...
                if (!locNorm.includes(selectedLocation.toLowerCase()))
                    return false;
            }
            if (selectedStatus !== 'All') {
                ...
                if (selectedStatus === 'resolved' && !isResolved)
                    return false;
                if (selectedStatus === 'urgent' && (isResolved || !(complaint.urgencyScore >= 0.75 || complaint.urgency === 'Urgent')))
                    return false;
                if (selectedStatus === 'under_review' && normStatus !== 'under_review')
                    return false;
                if (selectedStatus === 'submitted' && !isSubmitted)
                    return false;
            }
            return true;
        })
    ...
    return (<div className="max-w-7xl mx-auto py-8 sm:py-12 px-4 sm:px-6 lg:px-8">
      {/* Header Banner */}
      <div className="mb-10 border-b border-line pb-7 flex flex-col md:flex-row md:items-end justify-between gap-5">
        ...
        {!isAdminRole && (<button id="feed-lodge-grievance-btn" type="button" onClick={onGoToSubmit} className="s-btn s-btn-primary self-start md:self-auto shrink-0">
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: '-40px' }} transition={prefersReduced ? instantFade : paperSpring} className="bg-surface border border-line rounded-2xl p-5 sm:p-6 mb-10 shadow-soft">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <div>
            ...
            <div className="relative">
              ...
              <input id="feed-search-input" type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="Search ID, issue, or keyword..." className="s-input text-sm" style={{ paddingLeft: '2.5rem' }}/>
            </div>
          </div>

          <div>
            <label htmlFor="feed-cat-select" className="s-label">Category</label>
            <select id="feed-cat-select" value={selectedCategory} onChange={(e) => setSelectedCategory(e.target.value)} className="s-select text-sm cursor-pointer">
          ...
          <div>
            ...
            <select id="feed-status-select" value={selectedStatus} onChange={(e) => setSelectedStatus(e.target.value)} className="s-select text-sm cursor-pointer">
          ...
          <div>
            ...
            <select id="feed-loc-select" value={selectedLocation} onChange={(e) => setSelectedLocation(e.target.value)} className="s-select text-sm cursor-pointer truncate">
        ...
        <div className="mt-5 pt-4 border-t border-line flex flex-wrap items-center gap-2">
          ...
          <button type="button" onClick={() => setSelectedCategory('All')} className={`s-chip ${selectedCategory === 'All' ? 's-chip-active' : ''}`}>All</button>
          {CATEGORIES.map((cat) => (<button key={cat} type="button" onClick={() => setSelectedCategory(selectedCategory === cat ? 'All' : cat)} className={`s-chip ${selectedCategory === cat ? 's-chip-active' : ''}`}>
        ...
        <div className="mt-4 pt-4 border-t border-line flex flex-wrap items-center justify-between gap-3">
          <div className="flex items-center gap-2">
            ...
            <button type="button" onClick={() => setSortBy('upvotes')} className={`s-chip ${sortBy === 'upvotes' ? 's-chip-active' : ''}`}>Most Upvoted</button>
            <button type="button" onClick={() => setSortBy('newest')} className={`s-chip ${sortBy === 'newest' ? 's-chip-active' : ''}`}>Most Recent</button>
          </div>

          <div className="flex items-center gap-3">
            {(searchQuery || selectedCategory !== 'All' || selectedLocation !== 'All' || selectedStatus !== 'All') && (<button type="button" onClick={handleResetFilters} className="font-mono font-bold text-bronze-deep hover:underline cursor-pointer flex items-center gap-1 text-xs uppercase tracking-wide">
      ...
      {filteredComplaints.length > 0 ? (<motion.div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6" initial="hidden" animate="visible" layout variants={{ hidden: {}, visible: { transition: { staggerChildren: prefersReduced ? 0.06 : 0.06, delayChildren: prefersReduced ? 0 : 0.04 } } }}>
          <AnimatePresence mode="popLayout">
            {filteredComplaints.map((complaint) => (<ComplaintCard key={complaint.complaintId || complaint.id} complaint={complaint} onUpvote={onUpvote} onSelect={onSelectComplaint} onOpenImage={onOpenImage}/>))}
          </AnimatePresence>
        </motion.div>) : (<motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={prefersReduced ? instantFade : paperSpring} className="bg-surface border border-line rounded-2xl p-12 text-center max-w-xl mx-auto my-8 shadow-soft paper-grain">
          ...
          <div className="flex flex-wrap items-center justify-center gap-3">
              ...
              {!isAdminRole && (<button type="button" onClick={onGoToSubmit} className="s-btn s-btn-sm s-btn-secondary">Lodge New Grievance</button>)}
            </div>
        </motion.div>)}
    </div>);
};
