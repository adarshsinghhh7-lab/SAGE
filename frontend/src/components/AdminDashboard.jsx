...
import { Search, ArrowUpDown, CheckCircle2, Clock, AlertTriangle, ArrowBigUp, MapPin, Lock, Layers, PieChart as PieIcon, BarChart3, FileSpreadsheet, RotateCcw, Crown, Zap, Send } from 'lucide-react';
...
export const AdminDashboard = ({ complaints, onUpdateComplaint, onOpenImage, }) => {
    ...
    const [disputeOverrides, setDisputeOverrides] = useState({});
    ...
    const [escalationSettings, setEscalationSettings] = useState(null);
    ...
    const [resolutionLedgerTs, setResolutionLedgerTs] = useState({});
    useEffect(() => {
        let cancelled = false;
        (async () => {
            ...
            await Promise.all(missing.map(async (c) => {
                ...
                try {
                    ...
                    if (resolutionUpdate)
                        found[id] = resolutionUpdate.timestamp;
                }
                catch {
            ...
            if (!cancelled)
                setResolutionLedgerTs((prev) => ({ ...prev, ...found }));
        })();
        return () => {
            cancelled = true;
        };
    ...
    const locationChartData = useMemo(() => {
        ...
        complaints.forEach((c) => {
            ...
            const matched = OFFICIAL_LOCATIONS.find((official) => official.toLowerCase() === rawLower);
    ...
    const resolutionStats = useMemo(() => {
        ...
        const avgHours = resolvedCount > 0 ? (totalHours / resolvedCount).toFixed(1) : null;
    ...
    const filteredComplaints = useMemo(() => {
        return complaints
            ...
            if (selectedStatus !== 'All') {
                ...
                if (filterSt === 'urgent') {
                    if (complaint.urgencyScore < 0.75 || compSt === 'resolved')
                        return false;
                }
    ...
    const handleSaveModal = (id, newStatus, resolutionNotes) => {
    ...
    const confirmFlagDisputed = async (reasonText) => {
        ...
        try {
            ...
            setFlagError(/failed to fetch|networkerror|load failed|sealing server/i.test(raw)
    ...
    return (<motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={prefersReduced ? instantFade : paperSpring} className="max-w-7xl mx-auto py-8 sm:py-10 px-4 sm:px-6 lg:px-8 text-ink">
      {/* Header / Admin Banner */}
      <div className="mb-8 border-b border-line pb-6 flex flex-col md:flex-row md:items-end justify-between gap-4">
        ...
        <div className="flex items-center gap-3 shrink-0">
          ...
          <button type="button" onClick={openAuthModal} className="px-3 py-1.5 text-xs font-mono font-bold uppercase bg-surface hover:bg-surface-soft border border-line-strong rounded-lg cursor-pointer shadow-sm">
            Switch Role
          </button>
        </div>
      </div>
      ...
      {activeRole === 'head_admin' && (<div className="mb-6 bg-bronze-soft border border-line-strong p-4 shadow-soft flex items-center justify-between gap-4 flex-wrap">
          ...
          <button type="button" onClick={openAuthModal} className="px-3 py-1 bg-bronze text-ink font-mono text-xs font-bold uppercase hover:bg-bronze-deep cursor-pointer">
            Manage Claims
          </button>
        </div>)}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
          ...
          <div className="bg-surface border border-line-strong rounded-xl p-5 shadow-soft paper-grain">
            ...
            <div className="text-3xl sm:text-4xl font-mono font-bold text-ink mb-2 flex items-baseline gap-1.5">
              ...
              {resolutionStats.avgHours && (<span className="text-sm font-sans font-bold text-ink-soft">hours</span>)}
            </div>
          ...
          <div className="bg-clay-soft border border-clay/50 rounded-xl p-5 shadow-soft paper-grain">
            ...
            <div className="text-3xl sm:text-4xl font-mono font-bold text-clay-deep mb-2 flex items-baseline gap-2">
              ...
              {(statusStats.aiFlagged + statusStats.highPriority) > 0 && (<span className="w-2.5 h-2.5 rounded-full bg-clay animate-pulse"/>)}
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <div className="lg:col-span-5 bg-surface border border-line-strong rounded-xl p-5 shadow-soft paper-grain flex flex-col justify-between">
            ...
            <div className="h-64 sm:h-72 w-full flex items-center justify-center">
              {categoryChartData.length > 0 ? (<ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    <Pie data={categoryChartData} cx="50%" cy="50%" innerRadius={55} outerRadius={85} paddingAngle={3} dataKey="value">
                      {categoryChartData.map((entry, index) => (<Cell key={`cell-${index}`} fill={entry.color} stroke="#FFFFFF" strokeWidth={1}/>))}
                    </Pie>
                    <Tooltip formatter={(val) => [`${String(val)} complaints`, 'Total']} contentStyle={{
                ...
                </ResponsiveContainer>) : (<div className="text-xs font-mono text-ink-soft text-center">
          ...
          <div className="lg:col-span-7 bg-surface border border-line-strong rounded-xl p-5 shadow-soft paper-grain flex flex-col justify-between">
            ...
            <div className="h-64 sm:h-72 w-full">
              {locationChartData.length > 0 ? (<ResponsiveContainer width="100%" height="100%">
                  <BarChart data={locationChartData} margin={{ top: 10, right: 10, left: -15, bottom: 20 }}>
                    <CartesianGrid strokeDasharray="3 3" stroke="#E2E8F0"/>
                    <XAxis dataKey="location" stroke="#94A3B8" fontSize={10} fontFamily="monospace" angle={-20} textAnchor="end" interval={0}/>
                    <YAxis stroke="#94A3B8" fontSize={10} fontFamily="monospace" allowDecimals={false}/>
                    <Tooltip formatter={(val) => [`${String(val)} complaints`, 'Total']} contentStyle={{
                backgroundColor: '#FFFFFF',
                border: '1px solid #E2E8F0',
                borderRadius: '8px',
                ...
                boxShadow: '0 4px 12px rgba(15, 23, 42, 0.08)',
                    ...
                    <Bar dataKey="complaints" fill="#5F7A66" stroke="#5F7A66" strokeWidth={0} radius={[4, 4, 0, 0]}/>
                  </BarChart>
                </ResponsiveContainer>) : (<div className="text-xs font-mono text-ink-soft text-center flex items-center justify-center h-full">
    ...
    {/* ========================================================================= */}
      ...
      {/* ========================================================================= */}
      {activeRole !== 'student' && (<section className="mb-10">
          <div className="flex items-center justify-between gap-3 mb-4">
            <div className="flex items-center gap-2">
        ...
        {/* Threshold Control */}
              <div className="lg:col-span-1">
                ...
                <div className="flex items-stretch gap-2">
                  <input id="escalation-threshold" type="number" min={1} max={500} value={thresholdInput} onChange={(e) => setThresholdInput(e.target.value)} className="w-full bg-surface border border-line-strong rounded-lg px-3 py-2 text-sm font-mono font-bold text-ink focus:outline-none focus:bg-surface"/>
                  <button type="button" onClick={handleSaveThreshold} disabled={escalationSaving} className="px-3 py-2 bg-ink text-surface text-[11px] font-mono font-bold uppercase border border-line-strong rounded-lg hover:bg-moss-deep disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer whitespace-nowrap">
        ...
        {/* Manual Sweep Trigger */}
              <div className="lg:col-span-1">
                ...
                <button type="button" onClick={handleRunSweepNow} disabled={escalationRunning} className="w-full px-3 py-2 bg-bronze text-ink text-[11px] font-mono font-bold uppercase border border-line-strong hover:bg-bronze-deep disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer shadow-soft flex items-center justify-center gap-2">
        ...
        {/* Last Run Summary */}
              <div className="lg:col-span-1">
                ...
                {escalationSettings?.lastRun ? (<div className="border border-line-strong/20 bg-surface p-3 font-mono text-[11px] space-y-1">
                    ...
                    {escalationSettings.lastRun.escalated.length > 0 && (<div className="pt-1 border-t border-line text-[10px] text-ink-soft break-words">
                        {escalationSettings.lastRun.escalated
                        .map((entry) => `${entry.complaintId} → ${entry.newStatus}`)
                        .join(' · ')}
                      </div>)}
                  </div>) : (<div className="border border-dashed border-line-strong bg-surface p-3 text-[11px] font-mono text-ink-faint">
                    No sweep has been run yet.
                  </div>)}
                {escalationSettings?.lastRunAt && (<div className="text-[10px] font-mono text-ink-faint mt-1.5">
                    Last automated sweep: {formatTimestamp(escalationSettings.lastRunAt)}
                  </div>)}
              </div>
              </div>
        {/* Status / Feedback Message */}
            {escalationStatus && (<div className={`mt-4 px-3 py-2 border text-xs font-mono font-bold ${escalationStatus.type === 'success'
      ...
      <section>
        ...
        {disputeStatus && (<div className={"mb-4 px-3 py-2 border text-xs font-mono font-bold flex items-center gap-2 rounded-lg " + (disputeStatus.type === 'success' ? 'bg-accent/10 border-accent/40 text-accent-deep' : 'bg-clay-soft border-clay/40 text-clay-deep')} role="alert">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
            ...
            <div className="relative">
              ...
              <div className="relative">
                ...
                <input id="admin-search-input" type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="Filter by ID, issue, or notes..." className="w-full bg-surface border border-line-strong rounded-lg pl-9 pr-3 py-2 text-xs font-mono text-ink placeholder:text-ink-faint focus:outline-none focus:bg-surface"/>
              </div>
            </div>
            ...
            <div>
              ...
              <select id="admin-filter-cat" value={selectedCategory} onChange={(e) => setSelectedCategory(e.target.value)} className="w-full bg-surface border border-line-strong rounded-lg p-2 text-xs font-mono text-ink focus:outline-none cursor-pointer">
            ...
            <div>
              ...
              <select id="admin-filter-status" value={selectedStatus} onChange={(e) => setSelectedStatus(e.target.value)} className="w-full bg-surface border border-line-strong rounded-lg p-2 text-xs font-mono text-ink focus:outline-none cursor-pointer">
            ...
            <div>
              ...
              <select id="admin-filter-loc" value={selectedLocation} onChange={(e) => setSelectedLocation(e.target.value)} className="w-full bg-surface border border-line-strong rounded-lg p-2 text-xs font-mono text-ink focus:outline-none cursor-pointer truncate">
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 mt-3.5 pt-3 border-t border-line text-xs font-mono">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <div className="inline-flex gap-1">
                <button type="button" onClick={() => setSortBy('date-desc')} className={`px-2.5 py-1 text-[11px] uppercase font-bold border transition-colors cursor-pointer ${sortBy === 'date-desc'
            ? 'bg-ink text-surface border-line-strong'
            : 'bg-surface text-ink border-line-strong hover:bg-surface-soft'}`}>
                  Newest
                </button>
                <button type="button" onClick={() => setSortBy('upvotes-desc')} className={`px-2.5 py-1 text-[11px] uppercase font-bold border transition-colors cursor-pointer ${sortBy === 'upvotes-desc'
            ? 'bg-ink text-surface border-line-strong'
            : 'bg-surface text-ink border-line-strong hover:bg-surface-soft'}`}>
                  Most Upvotes
                </button>
                <button type="button" onClick={() => setSortBy('urgency-desc')} className={`px-2.5 py-1 text-[11px] uppercase font-bold border transition-colors cursor-pointer ${sortBy === 'urgency-desc'
            ? 'bg-ink text-surface border-ink'
            : 'bg-surface text-ink border-line-strong hover:bg-surface-soft'}`}>
                  AI Urgency
                </button>
                <button type="button" onClick={() => setSortBy('id-asc')} className={`px-2.5 py-1 text-[11px] uppercase font-bold border transition-colors cursor-pointer ${sortBy === 'id-asc'
            ? 'bg-ink text-surface border-line-strong'
            : 'bg-surface text-ink border-line-strong hover:bg-surface-soft'}`}>
                  ID Ascending
                </button>
            ...
            {(searchQuery || selectedCategory !== 'All' || selectedStatus !== 'All' || selectedLocation !== 'All') && (<button type="button" onClick={resetFilters} className="font-bold text-bronze-deep hover:underline cursor-pointer flex items-center gap-1 text-xs">
        ...
        <div className="bg-surface border border-line-strong rounded-xl shadow-lift overflow-x-auto">
          <table className="w-full text-left border-collapse font-mono text-xs">
            ...
            return (<tr key={compId} id={`admin-table-row-${compId}`} onClick={() => handleRowClick(complaint)} className={`hover:bg-surface-soft transition-colors cursor-pointer group ${isDisputed ? 'bg-[#B89B5E]/8 ring-1 ring-inset ring-[#B89B5E]/40' : ''}`}>
                      ...
                      <td className="py-3.5 px-4 whitespace-nowrap">
                        <div className="flex items-center gap-1.5 flex-wrap">
                          ...
                          {isAiFlagged && (<span className={`inline-flex items-center gap-1 px-2 py-0.5 text-[10px] font-bold uppercase border shadow-sm ${aiBadge.bg} ${aiBadge.text} ${aiBadge.border}`}>
                              <span className={`w-1.5 h-1.5 rounded-full ${aiBadge.indicator}`}/>
                              AI-Flagged Urgent
                            </span>)}

                          {/* High Priority badge (red) — upvote-based escalation */}
                          {isHighPriority && (<span className={`inline-flex items-center gap-1 px-2 py-0.5 text-[10px] font-bold uppercase border shadow-sm ${hpBadge.bg} ${hpBadge.text} ${hpBadge.border}`}>
                              <span className={`w-1.5 h-1.5 rounded-full ${hpBadge.indicator}`}/>
                              High Priority
                            </span>)}

                          {/* Disputed badge (amber) — admin-flagged suspicion */}
                          {isDisputed && (<span title={(disputeOverrides[compId]?.disputeReason || complaint.disputeReason) ? `Disputed: ${disputeOverrides[compId]?.disputeReason || complaint.disputeReason}` : 'Flagged by an admin as suspected false/malicious'} className={`inline-flex items-center gap-1 px-2 py-0.5 text-[10px] font-bold uppercase border shadow-sm ${dpBadge.bg} ${dpBadge.text} ${dpBadge.border}`}>
                    ...
                    handleRowClick(complaint);
                          ...
                          {disputeOverrides[compId]?.disputed === true || complaint.disputed === true ? (<button type="button" disabled className="inline-flex items-center gap-1 px-2 py-1 border border-bronze text-bronze-deep bg-bronze-soft text-[10px] uppercase font-bold whitespace-nowrap cursor-not-allowed" title={"Disputed - " + (disputeOverrides[compId]?.disputeReason || complaint.disputeReason || 'suspected false/malicious')}>
                              <AlertTriangle className="w-3 h-3"/>
                              Disputed
                            </button>) : (<button type="button" onClick={(e) => {
                        e.stopPropagation();
                        openFlagDisputed(complaint);
                    }} className="inline-flex items-center gap-1 px-2 py-1 bg-surface hover:bg-bronze hover:text-ink border border-line-strong transition-colors text-[10px] uppercase font-bold cursor-pointer whitespace-nowrap" title="Flag as suspected false/malicious - audited marking recorded on the complaint">
                              <AlertTriangle className="w-3 h-3"/>
                              Flag Disputed
                            </button>)}
                        </div>
                  ...
                  <td colSpan={7} className="py-12 px-4 text-center text-ink-soft bg-surface">
                    <div className="max-w-sm mx-auto">
                      ...
                      <button type="button" onClick={resetFilters} className="px-3 py-1 bg-ink text-surface text-xs uppercase font-mono font-bold border border-line-strong">
                        Reset All Filters
                      </button>
                    </div>
                  </td>
      ...
      <AdminComplaintModal complaint={modalComplaint} isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} onSave={handleSaveModal} onOpenImage={onOpenImage}/>

      {/* Flag-Disputed Modal */}
      <FlagDisputedModal complaintId={flagComplaint?.complaintId || flagComplaint?.id || ''} complaintRef={flagComplaint ? (flagComplaint.complaintId || flagComplaint.id) : undefined} isOpen={isFlagModalOpen} isSubmitting={flagSubmitting} submitError={flagError} onClose={() => {
            if (!flagSubmitting) {
                setIsFlagModalOpen(false);
                setFlagComplaint(null);
                setFlagError(null);
