...
import { Search, ArrowBigUp, MapPin, Lock, Crown, Key, ScrollText, ShieldCheck, ShieldAlert, Layers, RotateCcw, HelpCircle, } from 'lucide-react';
...
const revealLogContainerVariants = {
    hidden: {},
    show: {
        transition: { staggerChildren: 0.06 },
    },
};
const revealLogRowVariants = {
 ...
 */
export const HeadAdminDashboard = ({ complaints, onUpdateComplaint, onOpenImage, }) => {
    ...
    const [revealComplaint, setRevealComplaint] = useState(null);
    ...
    const handleRevealClick = (complaint) => {
    ...
    if (!isHeadAdmin) {
        return (<div className="max-w-3xl mx-auto py-16 sm:py-24 px-4 text-ink">
        <div className="bg-surface border border-line-strong rounded-2xl p-8 sm:p-10 shadow-lift paper-grain text-center">
          ...
          <button type="button" onClick={openAuthModal} className="px-6 py-3 bg-ink text-surface text-xs font-mono font-bold uppercase border border-line-strong rounded-lg hover:bg-moss-deep transition-colors cursor-pointer shadow-soft">
            Switch to Head Admin
          </button>
        </div>
      </div>);
    }
    return (<motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={prefersReduced ? instantFade : paperSpring} className="max-w-7xl mx-auto py-8 sm:py-10 px-4 sm:px-6 text-ink">
      {/* Header Banner */}
      ...
      <div className="mb-6 flex items-center gap-2 border-b border-line-strong">
        <button type="button" onClick={() => setActiveTab('complaints')} className={`px-4 py-2.5 text-xs font-mono font-bold uppercase tracking-wider transition-colors cursor-pointer border-b-2 ${activeTab === 'complaints'
            ? 'border-bronze text-bronze-deep bg-bronze-soft'
            : 'border-transparent text-ink-soft hover:text-ink'} flex items-center gap-1.5`}>
          <Layers className="w-3.5 h-3.5"/> Complaint Triage
        </button>
        <button type="button" onClick={() => setActiveTab('reveal-logs')} className={`px-4 py-2.5 text-xs font-mono font-bold uppercase tracking-wider transition-colors cursor-pointer border-b-2 ${activeTab === 'reveal-logs'
      ...
      {activeTab === 'complaints' ? (<section className="bg-surface border border-line-strong rounded-xl shadow-lift overflow-hidden">
          {/* Filters Header */}
          <div className="border-b border-line bg-surface p-4">
            <div className="flex flex-col md:flex-row gap-3">
              <div className="flex-1 relative">
                <Search className="w-4 h-4 text-ink-faint absolute left-3 top-1/2 -translate-y-1/2"/>
                <input type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="Search complaint ID, description, or location..." className="w-full bg-surface border border-line-strong rounded-lg pl-9 pr-3 py-2 text-xs font-mono text-ink placeholder:text-ink-faint focus:outline-none focus:border-bronze"/>
              </div>
              <div className="flex flex-wrap gap-2">
                <select value={selectedCategory} onChange={(e) => setSelectedCategory(e.target.value)} className="bg-surface border border-line-strong rounded-lg p-2 text-xs font-mono text-ink focus:outline-none cursor-pointer">
                  <option value="All">All Categories</option>
                  {ALL_CATEGORIES.map((cat) => (<option key={cat} value={cat}>{formatCategoryLabel(cat)}</option>))}
                </select>
                <select value={selectedStatus} onChange={(e) => setSelectedStatus(e.target.value)} className="bg-surface border border-line-strong rounded-lg p-2 text-xs font-mono text-ink focus:outline-none cursor-pointer">
                  <option value="All">All Statuses</option>
                  <option value="submitted">Submitted</option>
                  <option value="under_review">Under Review</option>
                  <option value="resolved">Resolved</option>
                </select>
                <select value={sortBy} onChange={(e) => setSortBy(e.target.value)} className="bg-surface border border-line-strong rounded-lg p-2 text-xs font-mono text-ink focus:outline-none cursor-pointer">
                  <option value="date-desc">Newest</option>
                  <option value="date-asc">Oldest</option>
                  <option value="upvotes-desc">Most Upvotes</option>
                  <option value="id-asc">ID Ascending</option>
                </select>
                {(searchQuery || selectedCategory !== 'All' || selectedStatus !== 'All') && (<button type="button" onClick={resetFilters} className="px-2.5 py-2 text-xs font-mono font-bold uppercase text-bronze-deep hover:underline cursor-pointer flex items-center gap-1">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse font-mono text-xs">
              ...
              <tbody className="divide-y divide-line-strong bg-surface">
                ...
                return (<tr key={compId} className={`hover:bg-surface-soft transition-colors cursor-pointer group ${isDisputed
                        ? 'bg-[#B89B5E]/8 ring-1 ring-inset ring-[#B89B5E]/40'
                        : ''}`}>
                        <td className="py-3.5 px-4 font-bold whitespace-nowrap">
                          <button type="button" onClick={() => handleRowClick(complaint)} className="bg-ink text-surface px-2 py-0.5 group-hover:bg-bronze transition-colors cursor-pointer">
                            {compId}
                          </button>
                        </td>
                        ...
                        <td className="py-3.5 px-4 whitespace-nowrap">
                          <div className="flex items-center gap-1.5 flex-wrap">
                            ...
                            {isDisputed && (<span title={complaint.disputeReason ? `Disputed: ${complaint.disputeReason}` : 'Flagged as suspected false/malicious'} className={`inline-flex items-center gap-1 px-2 py-0.5 text-[10px] font-bold uppercase border shadow-sm ${dpBadge.bg} ${dpBadge.text} ${dpBadge.border}`}>
                        ...
                        <td className="py-3.5 px-4 text-right whitespace-nowrap">
                          <div className="inline-flex items-center gap-1.5">
                            <button type="button" onClick={() => handleRowClick(complaint)} className="px-2.5 py-1 bg-surface hover:bg-ink hover:text-surface border border-line-strong transition-colors text-[10px] uppercase font-bold cursor-pointer">
                              Review
                            </button>
                            {activeTab === 'complaints' && isHeadAdmin && (<div className="relative inline-flex items-center">
                                <button type="button" onClick={() => handleRevealClick(complaint)} className="px-2.5 py-1 bg-clay hover:bg-clay-deep text-surface border border-clay transition-colors text-[10px] uppercase font-bold flex items-center gap-1 cursor-pointer" title="Head Admin only: decrypt & reveal submitter identity (permanently audited)">
                            ...
                            takes permanent page space. */}
                                <span className="relative inline-flex items-center group/reveal">
                                  <HelpCircle className="w-3.5 h-3.5 text-ink-faint hover:text-bronze-deep transition-colors cursor-help ml-1" aria-hidden="true"/>
                                  <span role="tooltip" className="pointer-events-none absolute right-0 top-full mt-2 z-30 w-64 bg-ink text-surface border border-line-strong rounded-lg p-3 text-left opacity-0 scale-95 translate-y-1 transition-all duration-150 group-hover/reveal:opacity-100 group-hover/reveal:scale-100 group-hover/reveal:translate-y-0 shadow-lift">
        ...
        <section className="bg-surface border border-line-strong rounded-xl shadow-lift overflow-hidden">
          <div className="border-b border-line bg-surface p-4 flex flex-wrap items-center justify-between gap-3">
            ...
            <button type="button" onClick={loadRevealLogs} disabled={logsLoading} className="px-3 py-1.5 bg-ink hover:bg-moss-deep text-surface text-[10px] font-mono font-bold uppercase border border-line-strong rounded cursor-pointer disabled:opacity-50">
              {logsLoading ? 'Refreshing...' : 'Refresh Ledger'}
            </button>
          </div>

          {logsError && (<div className="border-b border-clay/50 bg-clay-soft p-3 text-xs font-mono text-clay-deep font-bold">
              {logsError}
            </div>)}
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse font-mono text-xs">
              ...
              <motion.tbody className="divide-y divide-line-strong bg-surface" initial="hidden" animate="show" variants={revealLogContainerVariants}>
                {revealLogs.length > 0 ? (revealLogs.map((log) => (<motion.tr key={log.logId} variants={revealLogRowVariants} className="hover:bg-clay-deep/10 transition-colors">
                    ...
                    <td colSpan={4} className="py-12 px-4 text-center text-ink-soft bg-surface">
                      {logsLoading ? (<p className="font-sans text-sm">Loading reveal audit ledger...</p>) : (<p className="font-sans text-sm">
      ...
      {revealComplaint && (<RevealIdentityModal complaintId={revealComplaint.complaintId || revealComplaint.id || ''} complaintRef={revealComplaint.complaintId || revealComplaint.id} isOpen={isRevealModalOpen} onClose={() => setIsRevealModalOpen(false)} onRevealed={handleRevealed}/>)}
