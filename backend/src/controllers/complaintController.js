...
export class ComplaintController {
  ...
  static async create(req, res) {
    try {
      ...
      const normCategory = String(category).toLowerCase().replace('/', '_');
  ...
  static async upvote(req, res) {
    try {
      const { id } = req.params;
      const voterIdentifier = req.headers['x-sage-voter-id'] || req.user?.uid || req.ip;
  ...
  static async updateStatus(req, res) {
    try {
      ...
      const validStatuses = ['submitted', 'under_review', 'resolved'];
      const normStatus = String(status).toLowerCase().replace(' ', '_');
   ...
   */
  static async flagDisputed(req, res) {
    try {
      const { id } = req.params;
   ...
   */
  static async revealIdentity(req, res) {
    try {
      const { id } = req.params;
      const { reason } = req.body;
   ...
   */
  static async listRevealLogs(req, res) {
    try {
   ...
   */
  static sanitizeComplaint(complaint) {
    const { encryptedUserRef: _omitted, ...safe } = complaint;
    return safe;
  }
}
