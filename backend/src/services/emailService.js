...
function buildHtml(complaint, threshold, department) {
  const safe = (value) =>
  ...
  return [
    ...
    `<tr><td style="padding:2px 12px 2px 0;font-weight:bold;">Priority</td><td style="color:#4F46E5;font-weight:bold;">HIGH PRIORITY</td></tr>`,
    `</table>`,
    `<p style="margin:12px 0 4px;font-weight:bold;">Description</p>`,
    `<p style="margin:0;padding:10px;background:#F8FAFC;border:1px solid #E2E8F0;">${safe(complaint.description)}</p>`,
 ...
 */
export async function sendEscalationEmail(payload) {
  const to = payload.to;
