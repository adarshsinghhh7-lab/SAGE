import { db, isFirebaseLive } from '../config/firebaseAdmin.js';
import { encryptAES, decryptAES, hashSHA256 } from '../utils/crypto.js';
...
const SEED_COMPLAINTS = [
  {
    ...
    hostelOrLocation: 'Aryabhatta Block - 2nd Floor Corridor',
  ...
  },
  {
    ...
    isSandbox: true, // demo seed — simulated identity, NOT a real sealed record
    category: 'infrastructure',
    hostelOrLocation: 'Ramanujan Block - Lift 3',
  ...
  },
  {
    ...
    isSandbox: true, // demo seed — simulated identity, NOT a real sealed record
    category: 'harassment',
    hostelOrLocation: 'Vivekanand Bhavan - Main Gate Pathway',
  ...
  },
  {
    ...
    isSandbox: true, // demo seed — simulated identity, NOT a real sealed record
    category: 'wifi',
    hostelOrLocation: 'Bhabha Block - 1st Floor Quiet Study Area',
...
export function normalizeComplaint(doc) {
  ...
  const status = (data.status || 'submitted').toLowerCase().replace(' ', '_');
...
let inMemoryComplaints = new Map(
  SEED_COMPLAINTS.map((c) => [c.complaintId, normalizeComplaint(c)])
);
let inMemoryUpvotes = new Map();
let inMemoryStatusUpdates = [];
let inMemoryRevealLogs = [];

// In-Memory Escalation Settings (Admin-tunable threshold, persisted to Firestore when live)
let inMemoryEscalationSettings = new Map();

function getDefaultEscalationSettings() {
  return {
...
export class FirestoreService {
  ...
  static async getComplaints(filters) {
    ...
    if (isFirebaseLive && db) {
      try {
        let query = db.collection(this.collections.complaints);
        const snapshot = await query.get();
        complaints = snapshot.docs.map((doc) => normalizeComplaint(doc));
      } catch (err) {
   ...
   */
  static async upvoteComplaint(complaintId, voterIdentifier) {
  ...
  static async updateStatus(
    complaintId, newStatus, resolutionNotes,
    adminId = 'admin-user'
  ) {
    const complaint = await this.getComplaintById(complaintId);
    if (!complaint) return null;
    ...
    const updatedComplaint = normalizeComplaint({
      ...complaint,
      status: newStatus,
   ...
   */
  static async getComplaintsEligibleForEscalation(threshold) {
   ...
   */
  static async autoEscalate(complaintId) {
   ...
   */
  static async resetToDefaultSeed() {
   ...
   */
  static async getEscalationSettings() {
    if (isFirebaseLive && db) {
      try {
   ...
   */
  static async setEscalationThreshold(threshold, updatedBy) {
   ...
   */
  static async recordEscalationRun(report) {
    const current = await this.getEscalationSettings();
    const updated = {
      ...current,
