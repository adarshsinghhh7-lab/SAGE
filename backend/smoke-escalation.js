 ...
 * Run with:  node backend/smoke-escalation.js
 */
import { FirestoreService } from './src/services/firestoreService.js';
import { runAutoEscalation } from './src/services/escalationService.js';
...
async function main() {
  ...
  const sysUpdates = updates.filter((u) => u.updatedBy === 'system-auto-escalation');
  console.log(`system-auto-escalation updates for SAGE-3318: ${sysUpdates.length}`);
  sysUpdates.forEach((u) =>
