...
export const HeadAdminDashboard: React.FC<HeadAdminDashboardProps> = ({
  ...
  if (!isHeadAdmin) {
    return (
      <div className="max-w-3xl mx-auto py-16 sm:py-24 px-4 text-[#151820]">
        <div className="bg-[#EBE3D0] border border-[#DDD4BD] rounded-lg p-8 sm:p-10 shadow-lg text-center">
          <div className="w-14 h-14 mx-auto mb-5 bg-[#B59340] text-[#EBE3D0] flex items-center justify-center">
            <ShieldAlert className="w-7 h-7" />
          </div>
          <div className="inline-flex items-center gap-2 text-[10px] font-mono font-bold uppercase tracking-widest bg-[#0B0C0F] text-[#EBE3D0] px-2.5 py-1 mb-4">
          ...
          <p className="font-sans text-sm sm:text-base text-[#151820]/80 max-w-xl mx-auto mb-6">
          ...
          <div className="flex flex-wrap items-center justify-center gap-3 mb-6 text-xs font-mono">
            <span className="inline-flex items-center gap-1.5 border border-[#DDD4BD] rounded-lg px-3 py-1.5">
              Current Role: <strong className="uppercase">{activeRole.replace('_', ' ')}</strong>
            </span>
            <span className="inline-flex items-center gap-1.5 border border-[#DDD4BD] rounded-lg px-3 py-1.5">
              <Crown className="w-3.5 h-3.5 text-[#B59340]" /> Required:{' '}
          ...
          <button
            ...
            className="px-6 py-3 bg-[#0B0C0F] text-[#EBE3D0] text-xs font-mono font-bold uppercase border border-[#DDD4BD] rounded-lg hover:bg-[#B59340] transition-colors cursor-pointer shadow-md"
          >
  ...
  return (
    <div className="max-w-7xl mx-auto py-8 sm:py-10 px-4 sm:px-6 text-[#151820]">
      {/* Header Banner */}
      <div className="mb-6 bg-[#0B0C0F] text-[#EBE3D0] border border-[#DDD4BD] rounded-lg p-5 shadow-lg">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            <div className="w-11 h-11 bg-[#B59340] text-[#EBE3D0] flex items-center justify-center rounded-lg">
            ...
            <div>
              ...
              <p className="text-xs font-mono text-[#DDD4BD] mt-0.5">
                Immutable reveal audit + full grievance operations (Head Admin only)
              </p>
            </div>
          </div>
          <div className="inline-flex items-center gap-2 text-[10px] font-mono font-bold uppercase bg-[#B59340] text-[#151820] px-2.5 py-1 rounded">
      ...
      {/* Tab Switcher */}
      <div className="mb-6 flex items-center gap-2 border-b border-[#DDD4BD]">
        <button
          type="button"
          ...
          className={`px-4 py-2.5 text-xs font-mono font-bold uppercase tracking-wider transition-colors cursor-pointer border-b-2 ${
            activeTab === 'complaints'
              ? 'border-[#B59340] text-[#B59340] bg-[#B59340]/10'
              : 'border-transparent text-[#68707E] hover:text-[#151820]'
        ...
        >
          ...
          <span className="text-[10px] bg-[#A6352C] text-[#EBE3D0] px-1.5 py-0.2 rounded-full">
            {revealLogs.length}
          </span>
        </button>
      </div>
      ...
      {activeTab === 'complaints' ? (
        <section className="bg-[#EBE3D0] border border-[#DDD4BD] rounded-lg shadow-lg overflow-hidden">
          {/* Filters Header */}
          <div className="border-b border-[#DDD4BD] bg-[#EBE3D0] p-4">
            <div className="flex flex-col md:flex-row gap-3">
              <div className="flex-1 relative">
                <Search className="w-4 h-4 text-[#68707E]/60 absolute left-3 top-1/2 -translate-y-1/2" />
                <input
                  type="text"
                  value={searchQuery}
                  onChange={(e) => setSearchQuery(e.target.value)}
                  placeholder="Search complaint ID, description, or location..."
                  className="w-full bg-[#EBE3D0] border border-[#DDD4BD] rounded-lg pl-9 pr-3 py-2 text-xs font-mono text-[#151820] placeholder:text-[#68707E]/60 focus:outline-none focus:border-[#B59340]"
                />
              </div>
              <div className="flex flex-wrap gap-2">
                <select
                  ...
                  className="bg-[#EBE3D0] border border-[#DDD4BD] rounded-lg p-2 text-xs font-mono text-[#151820] focus:outline-none cursor-pointer"
                >
                ...
                {(searchQuery || selectedCategory !== 'All' || selectedStatus !== 'All') && (
                  <button
                    ...
                    className="px-2.5 py-2 text-xs font-mono font-bold uppercase text-[#B59340] hover:underline cursor-pointer flex items-center gap-1"
                  >
              ...
              <tbody className="divide-y divide-[#DDD4BD] bg-[#EBE3D0]">
                {filteredComplaints.length > 0 ? (
                  filteredComplaints.map((complaint) => {
                    ...
                    return (
                      <tr key={compId} className="hover:bg-[#EBE3D0] transition-colors cursor-pointer group">
                        <td className="py-3.5 px-4 font-bold whitespace-nowrap">
                          <button
                            type="button"
                            onClick={() => handleRowClick(complaint)}
                            className="bg-[#0B0C0F] text-[#EBE3D0] px-2 py-0.5 group-hover:bg-[#B59340] transition-colors cursor-pointer"
                          >
                            {compId}
                          </button>
                        </td>
                        ...
                        <td className="py-3.5 px-4 whitespace-nowrap text-[#151820]/80">
                          ...
                          <span className="ml-1 text-[10px] text-[#151820]/50">{formatTimeAgo(complaint.createdAt)}</span>
                        </td>
                        <td className="py-3.5 px-4 text-[#151820] max-w-[200px] truncate" title={location}>
                          ...
                          <div className="inline-flex items-center gap-1.5">
                            ...
                            {activeTab === 'complaints' && isHeadAdmin && (
                              <button
                                ...
                                className="px-2.5 py-1 bg-[#A6352C] hover:bg-[#A6352C] text-[#EBE3D0] border border-[#A6352C] transition-colors text-[10px] uppercase font-bold cursor-pointer flex items-center gap-1"
        ...
        // ---------------------------------------------------------------
        <section className="bg-[#EBE3D0] border border-[#DDD4BD] rounded-lg shadow-lg overflow-hidden">
          <div className="border-b border-[#DDD4BD] bg-[#EBE3D0] p-4 flex flex-wrap items-center justify-between gap-3">
            ...
            <button
              ...
              className="px-3 py-1.5 bg-[#0B0C0F] hover:bg-[#B59340] text-[#EBE3D0] text-[10px] font-mono font-bold uppercase border border-[#DDD4BD] rounded cursor-pointer disabled:opacity-50"
            >
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse font-mono text-xs">
              ...
              <motion.tbody
                className="divide-y divide-[#DDD4BD] bg-[#EBE3D0]"
                initial="hidden"
                animate="show"
              ...
              >
                {revealLogs.length > 0 ? (
                  revealLogs.map((log) => (
                    ...
                    >
                      <td className="py-3 px-4 font-bold whitespace-nowrap">
                        <span className="bg-[#A6352C] text-[#EBE3D0] px-2 py-0.5">{log.complaintId}</span>
                      </td>
                      <td className="py-3 px-4 text-[#151820] font-mono">
                        {log.revealedByAdminId}
                      </td>
                      <td className="py-3 px-4 text-[#151820] max-w-[320px]">
                        <span className="line-clamp-2">{log.reason}</span>
                      </td>
                      <td className="py-3 px-4 whitespace-nowrap text-[#151820]/80">
                  ...
                  <tr>
                    <td colSpan={4} className="py-12 px-4 text-center text-[#151820]/60 bg-[#EBE3D0]">
                      {logsLoading ? (
